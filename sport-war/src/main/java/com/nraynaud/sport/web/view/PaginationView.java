...
public class PaginationView<T, U> {
    ...
    private PaginationView(final PaginatedCollection<T> collection,
                           final PaginatedCollection.Transformer<T, U> transformer) {
        this.collection = collection;
        this.transformer = transformer;
    }

    public static <T, U> PaginationView<T, U> view(final PaginatedCollection<T> collection,
                                                   final PaginatedCollection.Transformer<T, U> transformer) {
        return new PaginationView<T, U>(collection, transformer);
    }

    public static <T> PaginationView<T, PaginatedCollection<T>> view(final PaginatedCollection<T> collection) {
        return view(collection, new PaginatedCollection.Transformer<T, PaginatedCollection<T>>() {
