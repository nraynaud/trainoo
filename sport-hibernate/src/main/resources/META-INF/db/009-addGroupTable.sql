...
ALTER TABLE `PUBLIC_MESSAGES` ADD COLUMN `GROUP_ID` bigint(20) DEFAULT NULL AFTER `IS_READ`;
ALTER TABLE `PUBLIC_MESSAGES` ADD KEY `messages_group_ID` USING BTREE (`GROUP_ID`);
ALTER TABLE `PUBLIC_MESSAGES` ADD CONSTRAINT `messages_group_fk` FOREIGN KEY (`GROUP_ID`) REFERENCES `GROUPS` (`ID`);
